...
const Card = (props) => {
	...
	return (
		<div>
			<div
				className={`card w-96 ${
					theme === "light" ? "bg-base-300 hover:bg-gray-300" : "bg-base-200"
				} shadow-2xl hover:scale-105 duration-300 transform-gpu
            hover:bg-base-300 xl:w-[30rem]`}
				onClick={() => {
					window.open(props.link, "_blank");
				}}
			>
				{props.image ? (
					<div className="bg-contain bg-no-repeat max-w-md max-h-44 lg:xl:max-h-60 xl:max-h-72 xl:max-w-full bg-left-top rounded-2xl mb-4 xl:h-72 overflow-clip">
				...
				<div className="card-body">
					...
					<p className={`text-2xl ${!props.description ? "hidden" : "h-36"}`}>
						{props.description}
					</p>
